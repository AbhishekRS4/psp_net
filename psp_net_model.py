...
class PSPNet:
    ...
    def psp_net_decoder(self):
        self.spp_out = self._spatial_pyramid_pool_block(
            self.stage5, name='spp_')

        self.decoder_conv = self._get_conv2d_layer(
            self.spp_out, 512, [3, 3], [1, 1], use_bias=False, name='decoder_conv')
        self.decoder_bn = self._get_batchnorm_layer(
            self.decoder_conv, name='decoder_bn')
        self.decoder_elu = self._get_elu_activation(
            self.decoder_bn, name='decoder_elu')
        self.decoder_dropout = self._get_dropout_layer(
            self.decoder_elu, rate=0.1, name='decoder_dropout')
    ...
    def _spatial_pyramid_pool_block(self, input_layer, name='spp_'):
        _interp_block1 = self.interp_block(
            input_layer, 1, name=name + 'interp_block1_')
        _interp_block2 = self.interp_block(
            input_layer, 2, name=name + 'interp_block2_')
        _interp_block3 = self.interp_block(
            input_layer, 3, name=name + 'interp_block3_')
        _interp_block6 = self.interp_block(
            input_layer, 6, name=name + 'interp_block6_')

        _concat_features = tf.concat([input_layer, _interp_block1, _interp_block2, _interp_block3,
                                      _interp_block6], axis=self._feature_map_axis, name=name + 'concat')
    ...
    def interp_block(self, input_layer, level, name):
        ...
        _conv = self._get_conv2d_layer(_avg_pool, 512, [1, 1], [
                                       1, 1], use_bias=False, name=name + 'conv')
        _bn = self._get_batchnorm_layer(_conv, name=name + 'bn')
        _elu = self._get_elu_activation(_bn, name=name + 'elu')

        _interp = self._get_upsample_layer(
            _elu, target_size=_feature_map_size, name=name + 'up')
    ...
    def _get_avg_pool_layer(self, input_layer, pool_size, strides, name='avg_pool'):
        return tf.layers.average_pooling2d(input_layer, pool_size, strides, data_format=self._data_format, name=name)
