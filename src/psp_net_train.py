...
from psp_net_model import PSPNet
...
def compute_loss(ground_truth, prediction, axis=1, name="mean_cross_entropy"):
    ...
    mean_ce = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(labels=ground_truth,
        logits=prediction), name=name
    )
...
def batch_train(FLAGS):
    ...
    psp_net.resnet50_encoder(features)
    print("resnet-50 encoder built")
    psp_net.psp_net_decoder()
    print("pspnet decoder built")
    logits = psp_net.logits
    ...
    np.save(
        os.path.join(os.getcwd(), model_dir, FLAGS.model_metrics),
        (losses_dict)
    )
